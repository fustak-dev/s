import { expect, window, spyOn, test as t } from "t-t";
import { s } from "./index.js";
import { button as buttonAttrs } from "./attrs.js";
...
const className = {
    className: "c4218071375 c3162900569 c551628051 c932719810 c1563729502 c953425557 c1563737885 c1197747141 c3507439765 c484032348 c2340731367 c867070755 c4195542496 c4274158499 c1333516654 c2249472333"
};
...
test("should return classes names", () => {
	expect(s({ props: stylesheet, attributes: buttonAttrs })).toBe(className);
});

test("should set the id", () => {
	expect(spySetAttribute.args[0]).toBe("id");
	expect(spySetAttribute.args[1]).toBe("fustak-s");
});

test("should get element by id fustak-s", () => {
	expect(spyGetElementById.args[0]).toBe("fustak-s");
...
test("should return only new styles", () => {
	...
	s({ props: { ...stylesheet, padding: "2rem" }, attributes: buttonAttrs });
